...
trait MyList[+A]:
  ...
  def size: Int = foldLeft(0)((b, a) => b + 1)
  ...
  def map[B](f: A => B): MyList[B] = reverse.
    foldLeft(MyNil: MyList[B])((b, a) => f(a) :: b)
  ...
  def coflatMap[B](f: MyList[A] => B): MyList[B] = reverse.
    foldLeft((MyNil, MyNil): (MyList[A], MyList[B]))((lalb, a) =>
      lalb match
        case (la, lb) => (a :: la, f(a :: la) :: lb)
      )._2

  def coflatten = coflatMap(identity)

  // convert to a Scala immutable List...
  def toList: List[A] = foldLeft(Nil: List[A])((la, a) => a :: la)
